...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    const onMessage = (msg: Message) => {
      ...
      setConversations((prev) =>
        prev.map((c) =>
          c.id === msg.conversationId ? { ...c, messages: [msg], updatedAt: msg.createdAt } as any : c
  ...
  const handleSend = () => {
    ...
    sendSocketMessage(active.id, input.trim());
    setInput("");
                ...
                {groupedMessages.map(({ date, msgs }) => (
                  <div key={date}>
                    ...
                    {msgs.map((msg, i) => {
                      ...
                      const showSender = !isMe && active.isGroup &&
                        (i === 0 || msgs[i - 1]?.senderId !== msg.senderId);
                      return (
                        <div key={msg.id} className={cn("flex", isMe ? "justify-end" : "justify-start", "mb-1")}>
