...
const emptyForm = {
  title: "", url: "", type: "PAPER", domain: "",
  levelId: -1, subLevelType: "", description: "", tags: "",
};
...
export default function AdminResources() {
  ...
  const [step, setStep] = useState(1); // 1=domain, 2=level, 3=sublevel, 4=details
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.domain || form.levelId < 0 || !form.subLevelType || !form.title || !form.url) {
    ...
    try {
      ...
      showToast(editingId ? "Resource updated!" : "Resource submitted for review! ⏳");
      resetForm();
      fetchResources();
    } catch (err: any) {
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px 24px", position: "relative", zIndex: 10 }}>
            ...
            >
              <div style={{
                ...
                <h3 style={{
                  ...
                  fontWeight: 700, color: "#f59e0b", margin: "0 0 20px 0",
                }}>
                    ...
                    {steps.map((s, i) => (
                      <div key={s} style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <div style={{
                          ...
                          background: step > i + 1 ? "#10b981" : step === i + 1 ? "#f59e0b" : "rgba(255,255,255,0.05)",
                          border: `1px solid ${step >= i + 1 ? "transparent" : "rgba(255,255,255,0.1)"}`,
                        ...
                        <span style={{
                          fontSize: "11px", color: step === i + 1 ? "#f59e0b" : "#555",
                          fontFamily: "'DM Mono', monospace",
                        ...
                        {i < steps.length - 1 && (
                          <div style={{
                            ...
                            background: step > i + 1 ? "#10b981" : "rgba(255,255,255,0.1)",
                            transition: "all 0.3s",
                          }} />
                        )}
                      </div>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  {(step === 1 || editingId) && !editingId && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <p style={{ color: "#888", fontSize: "13px", marginBottom: "16px" }}>
                        Select the domain this resource belongs to:
                      </p>
                        ...
                        {DOMAINS.map(d => (
                          <motion.div
                            ...
                            style={{
                              ...
                              background: form.domain === d.id ? `${d.color}15` : "rgba(255,255,255,0.03)",
                              border: `1px solid ${form.domain === d.id ? d.color + "50" : "rgba(255,255,255,0.08)"}`,
                              transition: "all 0.2s",
                            }}
                          >
                            <div style={{ fontSize: "11px", fontWeight: 700, color: d.color, fontFamily: "'Orbitron', sans-serif" }}>
                              {d.name}
                            </div>
                          </motion.div>
                        ))}
                  ...
                  {step === 2 && !editingId && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      <p style={{ color: "#888", fontSize: "13px", marginBottom: "16px" }}>
                        Select the level (0–12) for <span style={{ color: accentColor }}>{selectedDomain?.name}</span>:
                      </p>
                        ...
                        {Array.from({ length: 13 }, (_, i) => (
                          <motion.div
                            ...
                            style={{
                              ...
                              textAlign: "center",
                              background: form.levelId === i ? `${accentColor}15` : "rgba(255,255,255,0.03)",
                              border: `1px solid ${form.levelId === i ? accentColor + "50" : "rgba(255,255,255,0.08)"}`,
                            }}
                          >
                            <div style={{ fontSize: "16px", fontWeight: 700, color: form.levelId === i ? accentColor : "#fff", fontFamily: "'DM Mono', monospace" }}>
                              {i}
                            </div>
                        ...
                        {SUB_LEVELS.map(sl => (
                          <motion.div
                            ...
                            style={{
                              ...
                              background: form.subLevelType === sl.id ? `${accentColor}10` : "rgba(255,255,255,0.03)",
                              border: `1px solid ${form.subLevelType === sl.id ? accentColor + "40" : "rgba(255,255,255,0.08)"}`,
                              transition: "all 0.2s",
                            }}
                          >
                            ...
                            {form.subLevelType === sl.id && (
                              <span style={{ marginLeft: "auto", color: accentColor }}>✓</span>
                            )}
                          </motion.div>
                        ))}
                      </div>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      {!editingId && (
                        <div style={{
                          ...
                          <span style={{ fontSize: "10px", color: accentColor, fontFamily: "'DM Mono',monospace", fontWeight: 700 }}>
                            {selectedDomain?.name}
                          </span>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "16px" }}>
                        ...
                        {/* Show domain/level/sublevel as dropdowns when editing */}
                        {editingId && (
                          <>
                            <div>
                            ...
                            <div>
                              ...
                              <select value={form.subLevelType}
                                onChange={e => setForm(f => ({ ...f, subLevelType: e.target.value }))}
                                style={{ ...inputStyle, cursor: "pointer" }}>
                                {SUB_LEVELS.map(sl => (
                                  <option key={sl.id} value={sl.id}>{sl.icon} {sl.label}</option>
                                ))}
                              </select>
                            </div>
                          </>
                        ...
                        <div>
                          ...
                          <input value={form.tags}
                            ...
                            placeholder="verilog, beginner" style={inputStyle} />
                        </div>
                        <div style={{ gridColumn: "1 / -1" }}>
                      ...
                      <div style={{ display: "flex", gap: "12px", marginTop: "20px", alignItems: "center" }}>
                        {!editingId && (
                          <button type="button" onClick={() => setStep(3)}
                            style={{ background: "none", border: "none", color: "#666", cursor: "pointer", fontSize: "12px", padding: "11px 0" }}>
                            ← Back
                          </button>
                        )}
                        <motion.button
                      ...
                      <p style={{ color: "#555", fontSize: "11px", marginTop: "10px", fontFamily: "'DM Mono',monospace" }}>
                        ⏳ Resources go to Super Admin for approval before appearing on site.
                      </p>
                    </motion.div>
                  )}
                </form>
        ...
        <div style={{ display: "flex", gap: "12px", flexWrap: "wrap", marginBottom: "24px", alignItems: "center" }}>
          ...
          <select value={filterDomain} onChange={e => setFilterDomain(e.target.value)}
            style={{ padding: "7px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "#fff", fontSize: "12px", cursor: "pointer", outline: "none" }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px", color: "#555", fontSize: "14px" }}>
            No resources found. Click <strong style={{ color: "#f59e0b" }}>+ Add Resource</strong> to submit one.
          </div>
        ) : (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: "200px" }}>
                    ...
                    <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                      ...
                      <span style={{ background: "rgba(255,255,255,0.05)", borderRadius: "6px", padding: "2px 8px", fontSize: "10px", color: "#888", fontFamily: "'DM Mono',monospace" }}>
                        {sl?.icon} {sl?.label || r.subLevelType}
                      </span>
                      <span style={{ background: statusStyle.bg, borderRadius: "6px", padding: "2px 8px", fontSize: "10px", color: statusStyle.color, fontFamily: "'DM Mono',monospace" }}>
                        {statusStyle.label}
                      </span>
                    </div>
                    {r.status === "REJECTED" && r.rejectionReason && (
                      <div style={{ marginTop: "6px", fontSize: "11px", color: "#f87171", fontStyle: "italic" }}>
                  ...
                  <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                    {(r.status === "PENDING" || r.status === "REJECTED") && (
                      <button onClick={() => handleEdit(r)}
                        style={{ padding: "7px 14px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "8px", color: "#ccc", fontSize: "12px", cursor: "pointer" }}>
                        ✏️ Edit
                      </button>
                    )}
                    <button onClick={() => handleDelete(r.id)}
                      style={{ padding: "7px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: "8px", color: "#f87171", fontSize: "12px", cursor: "pointer" }}>
                      🗑️
                    </button>
                  </div>
                </motion.div>
