...
type SubLevelProgress = {
  ...
  claimedLevels: number[];   // ← tracks which levels had celebration claimed
  totalXp: number;
};
...
export default function ResearchPath() {
  ...
  const activeLevelData = activeLevelIdx !== null
    ? RESEARCH_SUB_LEVELS.find((d) => d.levelId === levels[activeLevelIdx].id)
    : null;
  ...
  return (
    <div className="min-h-screen pt-16 relative bg-black overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: "14px", flexWrap: "wrap" }}>
              <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: theme.primary, fontWeight: 700 }}>
                {levels.length} Levels × 4 Sub-levels
              </div>
              <div style={{ background: "rgba(234,179,8,0.1)", border: "1px solid rgba(234,179,8,0.25)", borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: "#eab308", fontWeight: 700 }}>
                +{progress.totalXp} XP Earned
              </div>
              <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "999px", padding: "4px 14px", fontSize: "11px", fontFamily: "'DM Mono', monospace", color: "#888" }}>
                {progress.completedSubLevels.length}/{levels.length * 4} Sub-levels
              </div>
            </div>
          </div>
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: isActive ? theme.gradient : status === "completed" ? "rgba(0,0,0,0.8)" : "rgba(0,0,0,0.6)",
                        border: `3px solid ${status === "locked" ? "rgba(255,255,255,0.1)" : isActive ? theme.primary : theme.border}`,
                        boxShadow: isActive ? `0 0 30px ${theme.glow}, 0 0 70px ${theme.glow}` : status === "active" ? `0 0 16px ${theme.glow}` : "none",
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        boxShadow: isActive ? `0 0 32px ${theme.glow}` : status !== "locked" ? `0 4px 20px ${theme.card}` : "none",
                        cursor: status === "locked" ? "not-allowed" : "pointer",
                        opacity: status === "locked" ? 0.6 : 1,
                        transition: "all 0.3s ease",
                        position: "relative", overflow: "hidden",
                        zIndex: isActive ? 26 : 1,
                      }}
                    ...
                    >
                      ...
                      {isActive && (
                        <div style={{ marginTop: "10px", display: "flex", alignItems: "center", gap: "6px", color: theme.primary, fontSize: "10px", fontFamily: "'DM Mono', monospace", fontWeight: 700 }}>
                          <Star style={{ width: "10px", height: "10px" }} /> Sub-levels expanded →
                        </div>
                      )}
                    </motion.div>
                  </motion.div>
      ...
      {/* Sub-level panel — rendered above blur overlay */}
      <AnimatePresence>
        {activeLevelIdx !== null && activeLevelData && activeLevel && (
          <ResearchSubLevelPanel
