...
// ── Verification sub-level types (same as RTL structure) ────────────────────
...
export default function VerificationPath() {
  ...
  // Load verification sub-levels from JSON
  useEffect(() => {
  ...
  const handleSubLevelComplete = (subLevelId: string, xp: number) => {
    setProgress((prev) => {
      ...
      return {
        ...prev,
        completedSubLevels: [...prev.completedSubLevels, subLevelId],
  ...
  const handleLevelComplete = (levelId: number, bonusXp: number) => {
    ...
    setProgress((prev) => ({
      ...
      claimedLevels: prev.claimedLevels.includes(levelId)
        ? prev.claimedLevels
        : [...prev.claimedLevels, levelId],
  ...
  const activeLevelData = activeLevelIdx !== null
    ? verSubLevels.find((d) => d.levelId === levels[activeLevelIdx].id)
    : null;
          ...
          <div className="relative">
            {levels.map((level, idx) => {
              ...
              const completedSubCount = levelSubData
                ? levelSubData.subLevels.filter((sl: any) =>
                    progress.completedSubLevels.includes(sl.id)
                  ).length
                : 0;
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      style={{
                        ...
                        background: isActive
                          ? `linear-gradient(135deg, ${theme.card}, rgba(0,0,0,0.5))`
                          : status === "completed" ? theme.card : "rgba(255,255,255,0.025)",
                        border: `1px solid ${isActive ? theme.primary : status === "locked" ? "rgba(255,255,255,0.05)" : theme.border}`,
                        boxShadow: isActive ? `0 0 30px ${theme.glow}` : status !== "locked" ? `0 4px 18px ${theme.card}` : "none",
                        cursor: status === "locked" ? "not-allowed" : "pointer",
